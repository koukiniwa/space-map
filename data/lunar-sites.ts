export interface LandingSite {
  id: string
  name: string
  country: string
  year: number
  status: "active" | "inactive" | "lost"
  lat: number
  lon: number
  description: string
  articleUrl?: string
  /** ISO date string of landing, e.g. "1969-07-20" */
  landingDate?: string
  /** Public-domain mission photo shown in the info panel */
  photoUrl?: string
  photoCredit?: string
}

export const landingSites: LandingSite[] = [
  // ── ソビエト連邦 ──────────────────────────────────────────
  {
    id: "luna-15",
    name: "Luna 15",
    country: "ソビエト連邦",
    year: 1969,
    landingDate: "1969-07-21",
    status: "lost",
    lat: 17.0,
    lon: 60.0,
    description: "Apollo 11と同時期に月を周回し、サンプルリターンを先取りしようとしたソ連の無人探査機。着陸直前に制御を失い危機の海に墜落。アポロ11号帰還の2日前に失敗した。",
    articleUrl: "https://en.wikipedia.org/wiki/Luna_15",
    photoUrl: "/images/missions/luna-15.jpg",
    photoCredit: "Soviet Space Program",
  },
  {
    id: "luna-9",
    name: "Luna 9",
    country: "ソビエト連邦",
    year: 1966,
    status: "inactive",
    lat: 7.13,
    lon: -64.37,
    landingDate: "1966-02-03",
    description:
      "史上初めて月面への軟着陸に成功したソ連の無人探査機。嵐の大洋に着陸し、月面のパノラマ写真を初めて地球に送信した。",
    articleUrl: "https://ja.wikipedia.org/wiki/%E3%83%AB%E3%83%8A9%E5%8F%B7",
    photoUrl: "/images/missions/luna-9.jpg",
    photoCredit: "Soviet Academy of Sciences / NASA",
  },
  {
    id: "luna-16",
    name: "Luna 16",
    country: "ソビエト連邦",
    year: 1970,
    landingDate: "1970-09-20",
    status: "inactive",
    lat: -0.68,
    lon: 56.3,
    description: "史上初の無人月サンプルリターンミッション。豊かの海に着陸し、月の土壌約101gを採取して地球に帰還した。無人機によるサンプル回収の可能性を初めて実証した歴史的ミッション。",
    articleUrl: "https://ja.wikipedia.org/wiki/%E3%83%AB%E3%83%8A16%E5%8F%B7",
    photoUrl: "/images/missions/luna-16.jpg",
    photoCredit: "Soviet Academy of Sciences",
  },
  {
    id: "luna-17",
    name: "Luna 17 / Lunokhod 1",
    country: "ソビエト連邦",
    year: 1970,
    landingDate: "1970-11-17",
    status: "inactive",
    lat: 38.28,
    lon: -35.0,
    description: "人類初の無人月面走行車「ルノホート1号」を月面に届けたミッション。雨の海に着陸し、ローバーは322日間で10.54kmを走行。月面の詳細な科学データを初めて長期にわたり収集した。",
    articleUrl: "https://ja.wikipedia.org/wiki/%E3%83%AB%E3%83%8E%E3%83%9B%E3%83%BC%E3%83%881%E5%8F%B7",
    photoUrl: "/images/missions/luna-17.jpg",
    photoCredit: "Soviet Academy of Sciences",
  },
  {
    id: "luna-21",
    name: "Luna 21 / Lunokhod 2",
    country: "ソビエト連邦",
    year: 1973,
    landingDate: "1973-01-15",
    status: "inactive",
    lat: 25.85,
    lon: 30.45,
    description: "「ルノホート2号」を月面に届けたミッション。レ・モニエ・クレーターに着陸し、ローバーは4ヶ月間で約39kmを走行。長年にわたり惑星探査車の走行距離記録を保持した。",
    articleUrl: "https://ja.wikipedia.org/wiki/%E3%83%AB%E3%83%8E%E3%83%9B%E3%83%BC%E3%83%882%E5%8F%B7",
    photoUrl: "/images/missions/luna-21.jpg",
    photoCredit: "Soviet Academy of Sciences",
  },
  {
    id: "luna-20",
    name: "Luna 20",
    country: "ソビエト連邦",
    year: 1972,
    landingDate: "1972-02-21",
    status: "inactive",
    lat: 3.57,
    lon: 56.55,
    description: "ソ連の無人サンプルリターン探査機。アポロニウス高地に着陸し、月の土壌約55gを採取して地球に帰還した。Luna 16に続くソ連2回目のサンプルリターン成功。",
    articleUrl: "https://en.wikipedia.org/wiki/Luna_20",
    photoUrl: "/images/missions/luna-20.png",
    photoCredit: "Soviet Academy of Sciences",
  },
  {
    id: "luna-24",
    name: "Luna 24",
    country: "ソビエト連邦",
    year: 1976,
    landingDate: "1976-08-18",
    status: "inactive",
    lat: 12.25,
    lon: 62.2,
    description: "ソ連最後の月面着陸ミッション。危機の海に着陸し、月の土壌約170gを採取して地球に帰還。その後44年間、月からのサンプルリターンは行われなかった。",
    articleUrl: "https://ja.wikipedia.org/wiki/%E3%83%AB%E3%83%8A24%E5%8F%B7",
    photoUrl: "/images/missions/luna-24.png",
    photoCredit: "Soviet Academy of Sciences",
  },
  // ── アメリカ ──────────────────────────────────────────────
  {
    id: "surveyor-1",
    name: "Surveyor 1",
    country: "アメリカ",
    year: 1966,
    landingDate: "1966-06-02",
    status: "inactive",
    lat: -2.47,
    lon: -43.34,
    description: "アメリカ初の月面軟着陸成功機。嵐の大洋に着陸し、1万枚以上の月面写真を送信。Apollo着陸地点の安全性を確認する先行調査を担った。",
    articleUrl: "https://en.wikipedia.org/wiki/Surveyor_1",
    photoUrl: "/images/missions/surveyor-1.jpg",
    photoCredit: "NASA",
  },
  {
    id: "surveyor-3",
    name: "Surveyor 3",
    country: "アメリカ",
    year: 1967,
    landingDate: "1967-04-20",
    status: "inactive",
    lat: -2.94,
    lon: -23.34,
    description: "嵐の大洋に着陸したアメリカの無人探査機。スコップで月面土壌を掘削し強度を測定した。後にApollo 12の宇宙飛行士が徒歩で訪れ機体部品を地球に持ち帰った。",
    articleUrl: "https://en.wikipedia.org/wiki/Surveyor_3",
    photoUrl: "/images/missions/surveyor-3.jpg",
    photoCredit: "NASA",
  },
  {
    id: "surveyor-5",
    name: "Surveyor 5",
    country: "アメリカ",
    year: 1967,
    landingDate: "1967-09-11",
    status: "inactive",
    lat: 1.41,
    lon: 23.18,
    description: "静かの海に着陸。アルファ散乱装置で月面土壌の化学組成を初めて分析し、玄武岩質であることを確認。後のApollo 11着陸地点に近い。",
    articleUrl: "https://en.wikipedia.org/wiki/Surveyor_5",
    photoUrl: "/images/missions/surveyor-5.png",
    photoCredit: "NASA",
  },
  {
    id: "surveyor-6",
    name: "Surveyor 6",
    country: "アメリカ",
    year: 1967,
    landingDate: "1967-11-10",
    status: "inactive",
    lat: 0.46,
    lon: -1.40,
    description: "月の中央部シヌス・メディイに着陸。エンジンを再点火して月面から2.5m跳び上がる実験（ホッピング）に成功し、月面からの離陸技術を実証した。",
    articleUrl: "https://en.wikipedia.org/wiki/Surveyor_6",
    photoUrl: "/images/missions/surveyor-6.png",
    photoCredit: "NASA",
  },
  {
    id: "surveyor-7",
    name: "Surveyor 7",
    country: "アメリカ",
    year: 1968,
    landingDate: "1968-01-10",
    status: "inactive",
    lat: -41.01,
    lon: -11.53,
    description: "Surveyorシリーズ最後の探査機。ティコ・クレーター近くの高地に着陸し、他のSurveyorとは異なる地質を調査。レーザー反射実験にも参加した。",
    articleUrl: "https://en.wikipedia.org/wiki/Surveyor_7",
    photoUrl: "/images/missions/surveyor-7.png",
    photoCredit: "NASA",
  },
  {
    id: "apollo-11",
    name: "Apollo 11",
    country: "アメリカ",
    year: 1969,
    status: "inactive",
    lat: 0.674,
    lon: 23.473,
    landingDate: "1969-07-20",
    description:
      "人類初の月面着陸を達成したミッション。1969年7月20日、ニール・アームストロングとバズ・オルドリンが静かの海に降り立ち、歴史に刻まれた。",
    articleUrl: "https://ja.wikipedia.org/wiki/%E3%82%A2%E3%83%9D%E3%83%AD11%E5%8F%B7",
    photoUrl: "/images/missions/apollo-11.jpg",
    photoCredit: "NASA / Neil A. Armstrong",
  },
  {
    id: "apollo-12",
    name: "Apollo 12",
    country: "アメリカ",
    year: 1969,
    status: "inactive",
    lat: -3.012,
    lon: -23.422,
    landingDate: "1969-11-19",
    description:
      "嵐の大洋に精密着陸したアポロ2回目の有人月面着陸ミッション。近くのサーベイヤー3号の部品を回収し、精度の高い着陸技術を実証した。",
    articleUrl: "https://ja.wikipedia.org/wiki/%E3%82%A2%E3%83%9D%E3%83%AD12%E5%8F%B7",
    photoUrl: "/images/missions/apollo-12.jpg",
    photoCredit: "NASA",
  },
  {
    id: "apollo-14",
    name: "Apollo 14",
    country: "アメリカ",
    year: 1971,
    status: "inactive",
    lat: -3.645,
    lon: -17.471,
    landingDate: "1971-02-05",
    description:
      "フラ・マウロ高地に着陸。アラン・シェパードとエドガー・ミッチェルが月面を歩行し、約42kgの月のサンプルを採取して帰還した。",
    articleUrl: "https://ja.wikipedia.org/wiki/%E3%82%A2%E3%83%9D%E3%83%AD14%E5%8F%B7",
    photoUrl: "/images/missions/apollo-14.jpg",
    photoCredit: "NASA",
  },
  {
    id: "apollo-15",
    name: "Apollo 15",
    country: "アメリカ",
    year: 1971,
    status: "inactive",
    lat: 26.132,
    lon: 3.634,
    landingDate: "1971-07-30",
    description:
      "ハドリー・リルに着陸。初めて月面車を使用し、アペニン山脈の麓で地質調査を実施。約77kgの岩石サンプルを採取した。",
    articleUrl: "https://ja.wikipedia.org/wiki/%E3%82%A2%E3%83%9D%E3%83%AD15%E5%8F%B7",
    photoUrl: "/images/missions/apollo-15.jpg",
    photoCredit: "NASA / David R. Scott",
  },
  {
    id: "apollo-16",
    name: "Apollo 16",
    country: "アメリカ",
    year: 1972,
    status: "inactive",
    lat: -8.973,
    lon: 15.498,
    landingDate: "1972-04-21",
    description:
      "デカルト高地に着陸した初の月の高地探査ミッション。ジョン・ヤングとチャールズ・デュークが月面車で広範囲を探索し、約95kgのサンプルを回収した。",
    articleUrl: "https://ja.wikipedia.org/wiki/%E3%82%A2%E3%83%9D%E3%83%AD16%E5%8F%B7",
    photoUrl: "/images/missions/apollo-16.jpg",
    photoCredit: "NASA / Charles M. Duke Jr.",
  },
  {
    id: "apollo-17",
    name: "Apollo 17",
    country: "アメリカ",
    year: 1972,
    status: "inactive",
    lat: 20.191,
    lon: 30.772,
    landingDate: "1972-12-11",
    description:
      "タウルス・リットロウ谷への最後のアポロ着陸ミッション。ジーン・サーナンとハリソン・シュミットが約110kgのサンプルを採取。現在も人類最後の月面歩行となっている。",
    articleUrl: "https://ja.wikipedia.org/wiki/%E3%82%A2%E3%83%9D%E3%83%AD17%E5%8F%B7",
    photoUrl: "/images/missions/apollo-17.jpg",
    photoCredit: "NASA / Eugene A. Cernan",
  },
  {
    id: "change-3",
    name: "嫦娥3号",
    country: "中国",
    year: 2013,
    status: "inactive",
    lat: 44.12,
    lon: 19.51,
    landingDate: "2013-12-14",
    description:
      "中国初の月面軟着陸に成功した探査機。雨の海北部に着陸し、玉兔（ぎょくと）ローバーを展開。1972年以来初めて月面を走行した探査車となった。",
    articleUrl: "https://ja.wikipedia.org/wiki/%E5%AB%A6%E5%A8%A53%E5%8F%B7",
    photoUrl: "/images/missions/change-3.jpg",
    photoCredit: "CNSA / CLEP",
  },
  {
    id: "change-4",
    name: "嫦娥4号",
    country: "中国",
    year: 2019,
    status: "active",
    lat: -45.46,
    lon: 177.6,
    landingDate: "2019-01-03",
    description:
      "史上初の月の裏側への軟着陸を達成。フォン・カルマン・クレーターに着陸し、玉兔2号ローバーが現在も探査活動を継続中。中継衛星「鵲橋」を介して通信を行っている。",
    articleUrl: "https://ja.wikipedia.org/wiki/%E5%AB%A6%E5%A8%A54%E5%8F%B7",
    photoUrl: "/images/missions/change-4.png",
    photoCredit: "CNSA / CLEP",
  },
  {
    id: "change-5",
    name: "嫦娥5号",
    country: "中国",
    year: 2020,
    status: "inactive",
    lat: 43.06,
    lon: -51.92,
    landingDate: "2020-12-01",
    description:
      "中国初のサンプルリターンミッション。呂姆克山近くに着陸し、約1.73kgの月のサンプルを採取して地球に持ち帰ることに成功した。",
    articleUrl: "https://ja.wikipedia.org/wiki/%E5%AB%A6%E5%A8%A55%E5%8F%B7",
    photoUrl: "/images/missions/change-5.jpg",
    photoCredit: "CNSA / CLEP",
  },
  {
    id: "change-6",
    name: "嫦娥6号",
    country: "中国",
    year: 2024,
    status: "inactive",
    lat: -41.64,
    lon: 153.97,
    landingDate: "2024-06-01",
    description:
      "月の裏側（南極エイトケン盆地のアポロ・クレーター）から史上初のサンプル採取・帰還に成功。約1.9kgのサンプルを地球に持ち帰った。",
    articleUrl: "https://ja.wikipedia.org/wiki/%E5%AB%A6%E5%A8%A56%E5%8F%B7",
    photoUrl: "/images/missions/change-6.jpg",
    photoCredit: "CNSA / CLEP",
  },
  {
    id: "chandrayaan-3",
    name: "Chandrayaan-3",
    country: "インド",
    year: 2023,
    status: "inactive",
    lat: -69.37,
    lon: 32.35,
    landingDate: "2023-08-23",
    description:
      "インド初の月面軟着陸成功ミッション。月の南極付近に着陸し、プラギャン・ローバーが14日間にわたって探査活動を実施。インドは世界で4番目に月面着陸に成功した国となった。",
    articleUrl: "https://ja.wikipedia.org/wiki/%E3%83%81%E3%83%A3%E3%83%B3%E3%83%89%E3%83%A9%E3%83%BC%E3%83%A4%E3%83%BC%E3%83%B33%E5%8F%B7",
    photoUrl: "/images/missions/chandrayaan-3.png",
    photoCredit: "ISRO",
  },
  {
    id: "slim",
    name: "SLIM",
    country: "日本",
    year: 2024,
    status: "inactive",
    lat: -13.32,
    lon: 25.24,
    description:
      "JAXAの小型月着陸実証機。2024年1月20日に月面着陸に成功し、日本は世界で5番目に月面着陸を達成した国となった。ピンポイント着陸技術を実証したが、太陽電池の向きが逆になるトラブルが発生した。",
    landingDate: "2024-01-20",
    articleUrl: "https://www.jaxa.jp/projects/explorations/slim/index_j.html",
    photoUrl: "/images/missions/slim.png",
    photoCredit: "JAXA",
  },
  {
    id: "beresheet",
    name: "Beresheet",
    country: "イスラエル",
    year: 2019,
    landingDate: "2019-04-11",
    status: "lost",
    lat: 32.59,
    lon: 19.37,
    description: "SpaceILと民間企業が開発したイスラエル初の月探査機。晴れの海への着陸を試みたが、最終降下中にエンジンが停止して墜落。民間資金のみで開発された初の月探査機として注目を集めた。",
    articleUrl: "https://ja.wikipedia.org/wiki/%E3%83%99%E3%83%AC%E3%82%B7%E3%83%BC%E3%83%88_(%E5%AE%87%E5%AE%99%E8%88%B9)",
    photoUrl: "/images/missions/beresheet.jpg",
    photoCredit: "SpaceIL / IAI",
  },
  {
    id: "chandrayaan-2",
    name: "Chandrayaan-2 Vikram",
    country: "インド",
    year: 2019,
    landingDate: "2019-09-07",
    status: "lost",
    lat: -70.881,
    lon: 22.784,
    description: "インドの月南極探査ミッション。着陸機「ヴィクラム」は最終降下中に通信が途絶え墜落した。軌道船は現在も周回を継続。この失敗を教訓にChandrayaan-3が2023年に成功した。",
    articleUrl: "https://ja.wikipedia.org/wiki/%E3%83%81%E3%83%A3%E3%83%B3%E3%83%89%E3%83%A9%E3%83%BC%E3%83%A4%E3%83%BC%E3%83%B32%E5%8F%B7",
    photoUrl: "/images/missions/chandrayaan-2.jpg",
    photoCredit: "ISRO",
  },
  {
    id: "hakuto-r-m1",
    name: "HAKUTO-R M1",
    country: "日本",
    year: 2023,
    landingDate: "2023-04-25",
    status: "lost",
    lat: 47.5,
    lon: 44.4,
    description: "日本のispace社が開発した民間月面着陸機（1号機）。アトラス・クレーターへの着陸を試みたが、降下中に高度計の誤作動で燃料を使い果たし墜落。商業月面輸送の先駆けとして道を切り開いた。",
    articleUrl: "https://ja.wikipedia.org/wiki/HAKUTO-R",
    photoUrl: "/images/missions/hakuto-r-m1.jpg",
    photoCredit: "ispace",
  },
  {
    id: "hakuto-r-m2",
    name: "HAKUTO-R M2",
    country: "日本",
    year: 2025,
    landingDate: "2025-06-05",
    status: "lost",
    lat: 60.4,
    lon: 10.2,
    description: "ispace社の2機目の月面着陸機「レジリエンス」。フリゴリス海への着陸を試みたが、動力降下中に制御を失い墜落。2回連続の失敗となったが、商業月面輸送実現に向けた貴重な運用データを収集した。",
    articleUrl: "https://ja.wikipedia.org/wiki/HAKUTO-R",
    photoUrl: "/images/missions/hakuto-r-m2.jpg",
    photoCredit: "ispace",
  },
  {
    id: "luna-25",
    name: "Luna 25",
    country: "ロシア",
    year: 2023,
    landingDate: "2023-08-19",
    status: "lost",
    lat: -57.865,
    lon: 61.361,
    description: "ソ連崩壊後初のロシア月面探査機（旧ソ連以来47年ぶり）。月南極付近への着陸を試みたが、軌道修正中に推力過剰で制御不能となり墜落した。",
    articleUrl: "https://ja.wikipedia.org/wiki/%E3%83%AB%E3%83%8A25",
    photoUrl: "/images/missions/luna-25.jpg",
    photoCredit: "Roscosmos",
  },
  {
    id: "nova-c-odysseus",
    name: "Nova-C Odysseus",
    country: "アメリカ（民間）",
    year: 2024,
    status: "inactive",
    lat: -80.13,
    lon: 1.44,
    landingDate: "2024-02-22",
    description:
      "インテュイティブ・マシンズ社が開発した商業月着陸機。2024年2月22日に月の南極付近に着陸し、民間企業として初の月面着陸成功を達成。着陸時に横転したが交信には成功した。",
    articleUrl: "https://ja.wikipedia.org/wiki/IM-1",
    photoUrl: "/images/missions/nova-c-odysseus.jpg",
    photoCredit: "Intuitive Machines / NASA",
  },
  {
    id: "im-2-athena",
    name: "IM-2 Athena",
    country: "アメリカ（民間）",
    year: 2025,
    landingDate: "2025-03-06",
    status: "lost",
    lat: -84.5,
    lon: 1.5,
    description: "インテュイティブ・マシンズ社の2機目の月着陸機。月南極付近のモンス・ムートンへの着陸を試みたが、着陸後に横転し通信が途絶。NASAのCLPSプログラムの一環で氷探査機器を搭載していた。",
    articleUrl: "https://en.wikipedia.org/wiki/IM-2",
    photoUrl: "/images/missions/im-2-athena.jpg",
    photoCredit: "Intuitive Machines / NASA",
  },
  {
    id: "blue-ghost",
    name: "Blue Ghost",
    country: "アメリカ（民間）",
    year: 2025,
    landingDate: "2025-03-02",
    status: "inactive",
    lat: 18.6,
    lon: 57.5,
    description: "ファイアフライ・エアロスペース社が開発した商業月着陸機。2025年3月に危機の海への着陸に成功。NASAの商業月面輸送サービス（CLPS）のもと10の科学機器を月面に届け、満月期を乗り越えて14日間以上稼働した。",
    articleUrl: "https://en.wikipedia.org/wiki/Blue_Ghost_Mission_1",
    photoUrl: "/images/missions/blue-ghost.png",
    photoCredit: "Firefly Aerospace / NASA",
  },
]
